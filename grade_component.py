...
from mcp_utils import read_config, read_manual


def grade_component(config, component_path):
    stid_col = config['student_id_col']
    auto_df = pd.read_csv(op.join(component_path, 'autograde.csv'))
    auto_marks = dict(zip(auto_df[stid_col], auto_df['Total']))
    broken_df = pd.read_csv(op.join(component_path, 'broken.csv'))
    broken = dict(zip(broken_df[stid_col], auto_df['Mark']))
    ...
    assert len(set(auto_marks).union(broken)) == 0
    grades = {}
    for student_id in keys0:
        manual_mark = sum(manuals[student_id] for m in manuals)
